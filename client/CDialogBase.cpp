...
//                  })");
    // 按钮事件
    connect(m_btnMin, &QPushButton::clicked, this, &CDialogBase::slotMin);
    connect(m_btnMax, &QPushButton::clicked, this, &CDialogBase::slotMaxRestore);
    connect(m_btnClose, &QPushButton::clicked, this, &CDialogBase::slotClose);
}

void CDialogBase::setWindowTitleText(const QString &text)
{
    m_titleLab->setText(text);
}

void CDialogBase::setWindowTitleIcon(const QIcon &icon, int size)
{
    m_iconLab->setPixmap(icon.pixmap(size, size));
}

QWidget *CDialogBase::contentsWidget()
{
    return m_contentWidget;
}

void CDialogBase::initSize()
{
    this->resize(m_contentWidget->width(),m_contentWidget->height() + m_titleBar->height());
}

void CDialogBase::slotMin()
{
...
void CDialogBase::slotMaxRestore()
{
    QWidget* parent = getParentWidget();
...
void CDialogBase::slotClose()
{
    QWidget* parent = getParentWidget();
    parent->close();
}

bool CDialogBase::isInTitleBar(const QPoint &pos)
{
    return pos.y() <= 40;
}

CDialogBase::ResizeDir CDialogBase::getResizeDirection(const QPoint &pos)
{
    int x = pos.x();
    int y = pos.y();
    int w = width();
...
void CDialogBase::setCursorByDir(ResizeDir dir)
{
    switch (dir)
    {
...
void CDialogBase::mousePressEvent(QMouseEvent *event)
{
...
void CDialogBase::mouseMoveEvent(QMouseEvent *event)
{
...
void CDialogBase::mouseReleaseEvent(QMouseEvent *event)
{
    m_isDrag = false;
    m_isResize = false;
...
void CDialogBase::leaveEvent(QEvent *event)
{
    setCursor(Qt::ArrowCursor);
    QWidget::leaveEvent(event);
}

QWidget *CDialogBase::getParentWidget()
{
    return this;
}
